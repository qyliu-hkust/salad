...
namespace pgm
{
    ...
    public:
        ...
        sdsl::rrr_vector<> signs_rrr; // signs by rrr compress for decode, not used in this version
        ...
        PGMIndex(RandomIt begin, RandomIt end):
            ...
            levels_offsets(),
            errorPointCount(0),
            corrections((n) * BIT_WIDTH(Epsilon) / 64 + 3), // set 3 to avoid insufficient memory
            signs(n),
            signs_rrr() {
        ...
        uint64_t segment_slope_significand_max() const {
            uint64_t max_slope_significand = 0;
        ...
        void normal_decode_query(std::vector<K> &output) {
        ...
        void simd_decode_512i_query(std::vector<K> &output){
            ...
            for (int i = 0; i < slope_significand_simd.size(); i++) { // the slope is int64_t, the corrections and intercept are int32_t, we should align them
                ...
                for (Covered_Value k = 0; k < key_nums_tmp; k++) {
                    if (k < key_nums)
                        output[first_tmp[k]] = result_int32[k];
                    else
                        output[first_tmp[k - key_nums] + 1] = result_int32[k];
                }
                ...
                for (Covered_Value j = 2; j < cover_length_tmp; j= j + 2) {
                    ...
                    for (Covered_Value k = 0; k < key_nums_tmp; k++) {
                        if (k < key_nums)
                            output[first_tmp[k] + j] = result_int32[k];
                        else
                            output[first_tmp[k - key_nums] + j + 1] = result_int32[k];
                    }
                }
                ...
                for (Covered_Value k = 0; k < key_nums; k++) {
                    ...
                    if (cover_length_tmp < covered) {
                        ...
                        for (Covered_Value pos = cover_length_tmp; pos < covered; pos++)
                            output[first_tmp[k] + pos] = ((slope_significand * pos) >> slope_exponent) + intercept + corrections_vector_residual[++correct_pointers];
                    }
                }
            }
        ...
        void free_memory(string simd_type = "simd") {
            ...
            } else if (simd_type == "simd_simple") {
                for (auto i = 0; i < corrections_simd.size(); i++) {
                    aligned_delete(corrections_simd[i]);
                }
                vector<Covered_Value> ().swap(cover_length);
                vector<Correction_Value*> ().swap(corrections_simd);
        ...
        std::unordered_map<Covered_Value, Covered_Value> query_result_map_init() {
            std::unordered_map<Covered_Value, Covered_Value> result_map(n);
            Covered_Value pointers = -1;
            for(int i = 0; i < slope_significand_simd.size(); i++) {
                pointers = -1; // ervery batch, we reset the pointers
                Covered_Value *first_tmp = first_simd[i];
                Covered_Value *cover_tmp = covered_simd[i];
                Covered_Value cover_length_tmp = cover_length[i];
        ...
        std::unordered_map<K, K> decode_result_map_init() {
            std::unordered_map<Covered_Value, Covered_Value> result_map;
        ...
        void simd_init(bool use_max = true) {
            ...
            for (typename std::vector<Segment>::iterator it = segments_sort.begin(); it + key_nums < std::prev(segments_sort.end()); it = it + key_nums) {
                ...
                sort(simd_segments.begin(), simd_segments.end()); // part sorted
                idx += key_nums;
        ...
        void create_corrections() {
             corrections_simd.resize(cover_length.size());
        ...
        std::vector<K> simd_decode_512i(bool warm_up=true){
            ...
            Covered_Value key_nums_tmp = key_nums * 2;
            alignas(align_val) Correction_Value *result_int32 = aligned_new<Correction_Value>(key_nums_tmp);
            conversion_time = slope_significand_simd.size();
            auto start1 = std::chrono::high_resolution_clock::now();
            for (int i = 0; i < slope_significand_simd.size(); i++) { // the slope is int64_t, the corrections and intercept are int32_t, we should align them
                Covered_Value cover_length_tmp = cover_length[i];
                if (cover_length_tmp == 0)
        ...
        static int64_t get_correction(const uint64_t* corrections_val, uint64_t n, uint64_t i, const sdsl::bit_vector& signs)  {
            ...
            if (correction == 0 && signs[i] == 1) // special case for Epsilon + 1
                return Epsilon + 1;
            else {
    ...
    struct PGMIndex<K, Epsilon, EpsilonRecursive, Floating>::Segment {
        ...
        Segment(const canonical_segment& cs,
            ...
            // auto [cs_significand, cs_exponent, cs_slope_float, cs_intercept] = cs.get_fixed_point_segment(first, INT_MAX); // 获取线段的斜率和截距，定点表示法
            ...
            if (first == n - 1) {
                ...
                slope_exponent = 0;
                slope_significand = 0;
                auto error = static_cast<int64_t> (data[first]) - approximate(first); // __int128 ->  int64_t
            ...
            for (auto p = first; p < j; p++) {
                ...
                if (error == (Epsilon + 1)) {
                    set_correction(corrections_ptr, n, p, 0);
                    signs[p] = 1;
                } else {
