            ...
            for (auto & index_count : idx_list) {
                ...
                while (in.peek() != EOF) {
                    { // this {} is used to destroy index every loop obviously
                        ...
                        std::visit([&in, &Epsilon_Data, &input_filename](auto &index) {
                            // Epsilon_Data
                            ...
                            // load first_pos_index
                            index.load_first_pos_index(input_filename + ".sp_tree");
                        }, variant_index);
                        pgm_enumerator<K> enumerator_tmp;
                        // std::vector<K> result1 = ori_data[index_count];
                        std::visit([&enumerator_tmp](auto &index) {
                            index.normal_init();
                            // enumerator_tmp.corrections_vector = index.corrections_vector;
                        ...
                        if (index_sequences.size() > branch_num) {
                            ...
                            while (intersection_idx < intersection_length) {
                                ...
                                if (query_id_idx == index_sequences.size()) {
                                    intercection_result[equal_result++] = candidate_posting;
                                }
                                intersection_idx++;
                            }
                        }
