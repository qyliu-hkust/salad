...
int main() {
    ...
    __m512i result;
    __m512i a1 = _mm512_set_epi64(8, 7, 6, 5, 4, 3, 2, 1);
    __m512i b1 = _mm512_set_epi64(16, 15, 14, 13, 12, 11, 10, 9);

    __m256i a = _mm512_cvtepi64_epi32(a1);
    __m256i b = _mm512_cvtepi64_epi32(b1);

    // 创建一个空的 __m512i 变量


    // 将第一个 __m256i 变量插入到 __m512i 变量的前半部分
    result = _mm512_inserti64x4(result, a, 0);

    // 将第二个 __m256i 变量插入到 __m512i 变量的后半部分
    result = _mm512_inserti64x4(result, b, 1);

    result = _mm512_inserti64x4(result, b, 0);
    result = _mm512_inserti64x4(result, a, 1);

    alignas(align_val) int32_t *result_int32 = aligned_new<int32_t>(16); // 存储结果
    _mm512_store_epi32(result_int32, result);
    for (int i = 0;i < 16;i++) {
            cout << result_int32[i] << endl;
    }
